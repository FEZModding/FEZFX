...
// Row 1 : Position (xy), Scale (zw)
// Row 2 : InvertR/G/B (xyz), ClipDark (w)
// Row 3 : FlipR/G/B (xyz), Unused (w)
// Row 4 : TcOffset (xy), Unused (z), Hide (w)
...
VS_OUTPUT VS(VS_INPUT input)
{
    ...
    position.xy = position.xy * data[0].zw + data[0].xy;
    position.xy = position.xy / VERTEX_SCALE - 1.0;
    output.Position = float4(position, 1.0) * !data[3].w;
    
    float2 texCoord = input.TexCoord;
    texCoord *= data[0].zw / TEXEL_SCALE;
    texCoord += data[3].xy;
    texCoord.y += 1.0 - data[0].w / TEXEL_SCALE;
    output.TexCoord = texCoord;
    
    output.InvertRGB_ClipDark = data[1];
    output.FlipRGB = data[2].xyz;
