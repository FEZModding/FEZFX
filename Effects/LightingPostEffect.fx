...
float4 PS_Dusk_Multiply(VS_OUTPUT input) : COLOR0
{
    ...
    float3 color = scale * (Fog_Color * 2.0 - 1.0) + 1.0;
...
{
    float maxComponent = max(max(Fog_Color.r, Fog_Color.g), Fog_Color.b);
    float3 normalized = (Fog_Color / maxComponent) - 1.0;
    float3 color = (0.125 * NightContribution * normalized) + 1.0;
